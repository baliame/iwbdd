...
import pygame
from pygame.locals import K_f, K_m, K_r
...
class Editor:
    ...
    def render_elements(self, wnd):
        ...
        pygame.draw.line(wnd.display, passive_color, (1500, 356), (1580, 356))
        ...
        if self.editing_mode not in EditingModeLock:
            ...
            if self.editing_mode == EditingMode.SIMULATION or self.editing_mode == EditingMode.FRAMEBYFRAME:
                ...
                render_collisions_text = self.font.render("Render as collisions", True, active_color if self.controller.render_collisions else passive_color, 0)
    ...
    def keydown(self, event, ml):
        if (self.editing_mode == EditingMode.SIMULATION or self.editing_mode == EditingMode.FRAMEBYFRAME):
            if event.key == Editor.key_mapping["reset_simulation"]:
                ...
                self.start_simulation()
                return
            if self.editing_mode == EditingMode.FRAMEBYFRAME and event.key == Editor.key_mapping["frame_advance"]:
